...
for i = 1:num_labels
  ...
  J = J + (sum( -1 * (y==i)' * log(pred2(:,i)) - (1-(y==i)') * log(1 - pred2(:,i)) ))/m;
endfor
...
Bigdelta1 = zeros(size(Theta1)); %25x401
...
for i=1:m
  ...
  z3 = Theta2*a2; % Theta2 : 10x26, a2 : 26x1, z3 : 10x1
  a3 = sigmoid(z3);
  ytemp = zeros(num_labels,1);
  ytemp(y(i)) = 1;
  
  delta3 = (a3 - ytemp); % Delta3  : 10x1
...
Bigdelta1(:,2:end) = Bigdelta1(:,2:end) + Theta1(:,2:end)*lambda/m;
Bigdelta2(:,2:end) = Bigdelta2(:,2:end) + Theta2(:,2:end)*lambda/m; 
